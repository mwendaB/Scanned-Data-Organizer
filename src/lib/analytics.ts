...
export class AnalyticsService {
  ...
  private static async getCurrentUserActivity(userId: string): Promise<Array<{ user: string; actions: number; documents: number }>> {
    ...
    const userEmail = userData.user?.email || 'You'

    return [{
      user: userEmail.split('@')[0],
      documents: docsData?.length || 0,
      actions: activityData?.length || 0
    }]
  }
  ...
  private static async getWorkspaceUserActivity(workspaceId: string): Promise<Array<{ user: string; actions: number; documents: number }>> {
    ...
    for (const member of members) {
      ...
      userActivity.push({
        user: member.user?.email?.split('@')[0] || 'Unknown',
        documents: docsData?.length || 0,
        actions: activityData?.length || 0
      })
    }

    return userActivity.sort((a, b) => (b.documents + b.actions) - (a.documents + a.actions))
  }
  ...
  private static async getWeeklyStats(filters: any): Promise<{
    ...
    const avgFileSize = allDocs?.reduce((sum, doc) => sum + doc.file_size, 0) / (allDocs?.length || 1) || 0
    ...
    const uniqueUsers = new Set(activeUserData?.map(doc => doc.user_id) || [])
